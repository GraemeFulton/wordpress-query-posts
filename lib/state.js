...
function requestPosts() {
	var query = arguments.length > 0 && arguments[1] !== undefined ? arguments[1] : {};
	...
	return function (dispatch) {
		...
		api.get('/wp/v2/' + postType, query).then(function (posts) {
			dispatch({
				type: POSTS_RECEIVE,
				posts: posts
			});
			requestPageCount('/wp/v2/' + postType, query).then(function (count) {
				dispatch({
...
function requestPost(postSlug) {
	return function (dispatch) {
		...
		api.get('/wp/v2/' + postType, query).then(function (data) {
			var post = data[0];
			dispatch({
				type: POSTS_RECEIVE,
				posts: [post]
